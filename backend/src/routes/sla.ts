 ...
 */
router.post('/:obligationId/extend', authenticate, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
 ...
 */
router.get('/:obligationId/history', authenticate, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const { obligationId } = req.params;
    const organizationId = req.user!.organization_id;
