 ...
 */
async function getCompleteObligationData(obligationId: string, organizationId: string): Promise<ObligationData | null> {
  // Get obligation
 ...
 */
router.get('/obligation/:id/pdf', authenticate, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
 ...
 */
router.get('/obligation/:id/zip', authenticate, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const organizationId = req.user!.organization_id;
    const userId = req.user!.id;
 ...
 */
router.get('/all/zip', authenticate, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const organizationId = req.user!.organization_id;
    const userId = req.user!.id;
